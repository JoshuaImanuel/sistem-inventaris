import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';

function DaftarInventaris() {
    const [daftarBarang, setDaftarBarang] = useState([]);
    const [kataKunci, setKataKunci] = useState('');

    useEffect(() => {
        fetch('http://localhost/sistem-inventaris/backend/get_all_items.php')
            .then(response => response.json())
            .then(data => {
                if (!data.error) {
                    setDaftarBarang(data);
                } else {
                    console.error(data.error);
                }
            })
            .catch(error => console.error('Galat komunikasi peladen:', error));
    }, []);

    const barangTersaring = daftarBarang.filter((barang) => {
        const kataKunciKecil = kataKunci.toLowerCase();
        return (
            barang.id.toLowerCase().includes(kataKunciKecil) ||
            barang.nama_alat.toLowerCase().includes(kataKunciKecil) ||
            barang.merek.toLowerCase().includes(kataKunciKecil) ||
            (barang.lokasi && barang.lokasi.toLowerCase().includes(kataKunciKecil))
        );
    });

    return (
        <div style={{ padding: '20px', maxWidth: '900px', margin: '0 auto', fontFamily: 'Arial, sans-serif' }}>
            <h2 style={{ color: '#0056b3', marginBottom: '20px' }}>Daftar Inventaris IT/MS</h2>
            <div style={{ marginBottom: '20px' }}>
                <input
                    type="text"
                    placeholder="Cari ID, Nama Alat, Merek, atau Lokasi..."
                    value={kataKunci}
                    onChange={(e) => setKataKunci(e.target.value)}
                    style={{ padding: '10px', width: '100%', boxSizing: 'border-box', border: '1px solid #ccc', borderRadius: '4px' }}
                />
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}>
                <thead>
                    <tr style={{ backgroundColor: '#0056b3', color: 'white' }}>
                        <th style={{ padding: '12px', border: '1px solid #ddd' }}>ID Alat</th>
                        <th style={{ padding: '12px', border: '1px solid #ddd' }}>Nama Alat</th>
                        <th style={{ padding: '12px', border: '1px solid #ddd' }}>Merek</th>
                        <th style={{ padding: '12px', border: '1px solid #ddd' }}>Lokasi</th>
                        <th style={{ padding: '12px', border: '1px solid #ddd' }}>Kondisi</th>
                        <th style={{ padding: '12px', border: '1px solid #ddd' }}>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    {barangTersaring.map((barang) => (
                        <tr key={barang.id} style={{ backgroundColor: '#fff' }}>
                            <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center' }}>{barang.id}</td>
                            <td style={{ padding: '12px', border: '1px solid #ddd' }}>{barang.nama_alat}</td>
                            <td style={{ padding: '12px', border: '1px solid #ddd' }}>{barang.merek}</td>
                            <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center' }}>{barang.lokasi}</td>
                            <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center', color: barang.kondisi === 'Baik' ? 'green' : 'red', fontWeight: 'bold' }}>
                                {barang.kondisi}
                            </td>
                            <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center' }}>
                                <Link to={`/item/${barang.id}`} style={{ color: '#0056b3', textDecoration: 'none', fontWeight: 'bold' }}>Lihat Detail</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
            </table>
        </div>
    );
}

export default DaftarInventaris;